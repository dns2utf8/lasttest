...
pub fn main_local(args: &::Args, pool: &ThreadPool) {
    if args.cmd_static {
        ...
        println!(
            "Static finished <=> pool.active_count() = {} // duration = {:?}\n",
            pool.active_count(),
            duration
        );
    }
    ...
    if args.cmd_communicating {
        ...
        let duration = start.elapsed();
        println!(
            "Communicating finished <=> pool.active_count() = {} // duration = {:?}\n",
            pool.active_count(),
            duration
        );
    }
    ...
    if args.cmd_chain {
        ...
        let duration = start.elapsed();
        println!(
            "Chain finished <=> pool.active_count() = {} // duration = {:?}\n",
            pool.active_count(),
            duration
        );
    }
    ...
    if args.cmd_flood {
        ...
        let duration = start.elapsed();
        println!(
            "Flood finished <=> pool.active_count() = {} // duration = {:?}\n",
            pool.active_count(),
            duration
        );
    }
    ...
    if args.cmd_mesh {
        ...
        let duration = start.elapsed();
        println!(
            "Mesh finished <=> pool.active_count() = {} // duration = {:?}\n",
            pool.active_count(),
            duration
        );
    }
...
pub fn run_static(pool: &ThreadPool) {
    ...
    for i in 0..TEST_TASKS {
        ...
        pool.execute(move || {
            let s = match j {
                ...
                3 => {
                    let start = Instant::now();
                    let r = pi_approx_random(versuche as u64, &mut rand::thread_rng());
                    let duration = start.elapsed();
                    println!(
                        "static pi_approx_random finished <=> duration = {:?} // {}\n",
                        duration,
                        format_pi_approx(r)
                    );
                    13
    ...
    println!(
        "Result {}",
        rx.iter().take(TEST_TASKS).fold(0, |a, b| a + b)
    );
}
...
pub fn run_communicating(pool: &ThreadPool) {
    ...
    for _ in 0..TEST_TASKS {
        ...
        pool.execute(move || {
            sender
                .send(pi_approx_random(versuche, &mut rand::thread_rng()))
                .unwrap()
        });
    }

    println!(
        "Result run_communicating: {}",
        receiver
            .iter()
            .take(TEST_TASKS)
    ...
    for b in v {
        ...
        if h == 0 {
            ...
            for f in 2..10000000 {
                ...
                if r == 0 {
                    // should never be true, but can not be known at compile time
...
fn float_steps() {
    ...
    println!(
        "float_steps.pi: {}",
        format_pi_approx((inside, inside + outside))
    );
}

/// (inside : u64, tries : u64)
fn pi_approx_random<R: Rng>(tries: u64, rng: &mut R) -> (u64, u64) {
    let mut inside = 0;
...
pub fn run_chain(pool: &ThreadPool) {
    ...
    for _ in 0..TEST_TASKS {
        ...
        pool.execute(move || {
            ...
            tx_chain.send((b.0 + r.0, b.1 + r.1)).unwrap();
        });
    }
    ...
    for _ in 0..TEST_TASKS {
        ...
        pool.execute(move || {
            ...
            for _ in 0..VERSUCHE {
                ...
                if x * x + y * y > 1.0 {
                    outside.send(Some(1)).unwrap();
                } else {
                    inside.send(Some(1)).unwrap();
                }
            }

            outside.send(None).unwrap();
            inside.send(None).unwrap();
        });
    }
...
pub fn run_mesh(pool: &ThreadPool) {
    let versuche = VERSUCHE;
    let v: Vec<usize> = vec![
        23, 42, 666, 521, 8192, 0x056b, 0xe660, 0x4c74, 0x8ca5, 0x12f6, 0xb000, 0x0a14, 0x40ee,
        0xa224, 0x2b36, 0x9d11, 0x26ab, 0xc2d2, 0xbd39, 0x5d63, 0x9e93, 0x1fd3, 0x55b4, 0x168f,
        0xff9b, 0x3ee2, 0x0342, 0x1cd3, 0x8782, 0xb321, 0x14e3, 0x7aeb, 0xea8e, 0xd8d2, 0xe43f,
        0xd8ee, 0x7a50, 0x3990, 0x7137, 0xe668, 0xceae, 0x2bfd, 0xd99e, 0x974d, 0x855d, 0xf4dd,
        0xe8db, 0x31a7, 0x4388, 0x6907, 0x283f, 0x3ae2, 0xf9d6, 0x6987, 0x235d, 0xdaa9, 0x6e0f,
        0x3ca8, 0xccab, 0x732f, 0x29e3, 0x4daf, 0xd6cd, 0xf463, 0x6786, 0xc835, 0x9e5b, 0xd5e6,
        0x97e1, 0x0675, 0x83eb, 0x3392, 0x5cda, 0x8c4b, 0x72e9, 0xed2f, 0x4042, 0x8bfc, 0x2f52,
        0xae13, 0x7666, 0x65c7, 0x2300, 0x50fc, 0xc6f6, 0xed82, 0xa637, 0xca83, 0x2b08, 0x95b1,
        0x67c0, 0x954c, 0x060c,
    ];
    let n_v = v.len();
        ...
        pool.execute(move || {
            ...
            if h == 0 {
                ...
                for f in 2..10_000_000 {
                    ...
                    if r == 0 {
                        // should never be true, but can not be known at compile time
                        tx.send(format!("{} times {} is 0", b, f)).unwrap();
                    }
                }
            }

            tx.send(format!("{}", h)).unwrap();
        });
    }
    ...
    for _ in 0..n_v {
        ...
        pool.execute(move || {
            ...
            let r = pi_approx_random(versuche as u64, &mut rng);
            tx3.send(Some(r)).unwrap();
        });
    }
    ...
    while pool.active_count() > 0 || i > 0 {
        if let Ok(n) = rx3.try_recv() {
            ...
            sleep(666);
            println!(
                "Waiting in SpinnLock ... remaining numbers: {}, pool: {}",
                i,
                pool.active_count()
            );
        }
